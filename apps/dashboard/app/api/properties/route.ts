...
import { propertyNumber } from '@/lib/property-input';
...
function propertyInput(body: Record<string, unknown>): PropertyInput {
  ...
  return {
    ...
    meta: clean(body.meta, 500),
    tone: clean(body.tone, 30) || 'orchid', purpose: body.purpose === 'Aluguel' ? 'Aluguel' : 'Venda', images: cleanImages(body.images),
    propertyType: clean(body.propertyType, 80), bedrooms: propertyNumber(body.bedrooms, true), parkingSpaces: propertyNumber(body.parkingSpaces, true), area: propertyNumber(body.area), publicUrl: clean(body.publicUrl, 500),
