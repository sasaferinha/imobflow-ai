...
export default function LicenseAdminClient() {
  ...
  return <main className="access-page"><section className="access-card" aria-labelledby="license-title">
    ...
    <div className="access-content"><div className="access-intro"><p className="access-eyebrow">ADMINISTRAÇÃO IMOBFLOW</p><h1 id="license-title">Chave do plano Basic.</h1><p>Após a confirmação do pagamento, gere uma chave para ativar uma imobiliária com um administrador e até cinco corretores.</p></div>
      <form onSubmit={submit} className="access-form admin-license-form"><div className="access-fields"><label>Plano contratado<input value="Basic · 1 administrador + 5 corretores" readOnly aria-readonly="true" /></label><label>Senha administrativa<input name="managementPassword" type="password" required autoComplete="current-password" /></label></div>{message && <p className="access-error" role="alert">{message}</p>}<button className="access-submit" type="submit" disabled={loading}>{loading ? 'Gerando chave…' : 'Gerar chave do Basic'}<span aria-hidden="true">→</span></button></form>
