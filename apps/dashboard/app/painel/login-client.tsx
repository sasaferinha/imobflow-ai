...
export default function LoginClient() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(`/api/account/${mode}`, {
      ...
      body: JSON.stringify({ company: form.get('company'), name: form.get('name'), email: form.get('email'), password: form.get('password'), accessKey: form.get('accessKey') }),
    }).catch(() => null);
  ...
  return <main className="access-page"><section className="access-card" aria-labelledby="access-title">
    ...
    <div className="access-content">
      <div className="access-intro"><p className="access-eyebrow">ACESSO DA IMOBILIÁRIA</p><h1 id="access-title">{registering ? 'Cadastre sua imobiliária.' : 'Acesse sua equipe.'}</h1><p>{registering ? 'Use a chave recebida após a contratação para criar o painel de administrador da sua empresa.' : 'Entre com o e-mail e a senha que o administrador da imobiliária entregou a você.'}</p></div>
      ...
      <form onSubmit={submit} className="access-form">
        ...
        <div className="access-fields"><label>Nome da empresa<input name="company" autoComplete="organization" placeholder="Ex.: Imobiliária Central" required minLength={2} maxLength={120} /></label>{registering && <label>Nome do administrador<input name="name" autoComplete="name" placeholder="Nome completo" required minLength={2} maxLength={120} /></label>}<label>{registering ? 'E-mail do administrador' : 'E-mail do corretor'}<input name="email" type="email" autoComplete="email" placeholder="nome@imobiliaria.com.br" required maxLength={254} /></label><label>{registering ? 'Senha do administrador' : 'Senha do corretor'}<input name="password" type="password" autoComplete={registering ? 'new-password' : 'current-password'} placeholder="Mínimo de 8 caracteres" required minLength={8} maxLength={128} /></label></div>
