...
export default function LoginClient() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch(recovering ? '/api/account/password' : `/api/account/${mode}`, {
      ...
      body: JSON.stringify({ action:'request', company: form.get('company'), name: form.get('name'), email: form.get('email'), password: form.get('password'), accessKey: form.get('accessKey') }),
    }).catch(() => null);
  ...
  return <main className="access-page"><section className="access-card" aria-labelledby="access-title">
    ...
    <div className="access-content">
      <div className="access-intro"><p className="access-eyebrow">ACESSO DA IMOBILIÁRIA</p><h1 id="access-title">{recovering ? 'Recupere seu acesso.' : registering ? 'Cadastre sua imobiliária.' : administrator ? 'Acesso do administrador.' : 'Acesso do corretor.'}</h1><p>{recovering ? 'Informe sua empresa e seu e-mail para receber um link de redefinição de senha.' : registering ? 'Use a chave recebida após a contratação para criar o painel de administrador da sua empresa.' : 'Entre com o nome da empresa, seu e-mail e sua senha pessoal.'}</p></div>
